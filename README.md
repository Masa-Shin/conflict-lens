...
ベースブランチ側で起きている変更と、自分のブランチでマージ時に衝突するであろう箇所を、エディタとファイルツリーに直接見せる VSCode 拡張。

> **長く動かしてるフィーチャブランチや、PR レビューの相手側ブランチで何が起きてるか** を、ターミナルで `git fetch && git diff` を叩かなくてもエディタの中で常時把握できるようにする。

```
 1  function greet(name) {
 2▌   return "Hi, " + name;        ◀ 赤背景 + 三角  : マージしたら衝突する
 3  }
 4
 5  function farewell(name) {
 6▌   return "Bye, " + name;       ◀ 黄色背景 + 縦棒: base 側がこの行を変えた
 7  }
```
